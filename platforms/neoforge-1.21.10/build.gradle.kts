...
extensions.configure<SourceSetContainer> {
    named("main") {
        ...
        java.srcDir("../shared/neoforge-player-gameprofile-name/src/main/java")
    }
}
...
extensions.configure<NeoForgeExtension>("neoForge") {
    ...
    runs {
        create("neoForge12110Client") {
            client()
            sourceSet.set(sourceSets.main.get())
            gameDirectory.set(layout.projectDirectory.dir("runs/neoForge12110Client"))
        }

        create("neoForge12110Server") {
            server()
            sourceSet.set(sourceSets.main.get())
            gameDirectory.set(layout.projectDirectory.dir("runs/neoForge12110Server"))
        }
    }
}
